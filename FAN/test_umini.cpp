// g++ -o test_umini test_umini.cpp -std=c++11
//sudo ./test_umini
...
int main() {
    printf("================================================\n");
    printf("      UMini-WS 超声波风速传感器单独测试\n");
    printf("================================================\n");
    
    // 打开串口
    int fd = open("/dev/ttyS3", O_RDWR | O_NOCTTY | O_NDELAY);
    if (fd < 0) {
        printf("错误: 无法打开串口 /dev/ttyS3 (errno=%d)\n", errno);
        return -1;
    }
    printf("串口打开成功, fd=%d\n", fd);
            ...
            if (bytes_read > 0) {
                ...
                if (bytes_read >= 7 && response[0] == 0x01 && response[1] == 0x03) {
                    if (response[2] == 4) {
                        ...
                        float speed = speed_raw * 0.01f;
                        float dir = dir_raw * 1.0f;
                        
                        printf("%-6d  %-10.2f  %-8.1f  ", sample_count + 1, speed, dir);
